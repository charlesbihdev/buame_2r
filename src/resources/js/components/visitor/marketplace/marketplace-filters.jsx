...
export function MarketplaceFilters({ filters = {}, isMobile = false }) {
    ...
    return (
        <aside className={`min-h-[calc(100vh-80px)] w-full shrink-0 flex-col gap-8 border-r border-[var(--buame-border-light)] bg-white p-6 ${isMobile ? 'flex' : 'hidden lg:flex lg:w-80'}`}>
            ...
            {(filters.category && filters.category !== 'all') ||
                ...
                <div>
                    <div className="mb-2 flex items-center justify-between">
                        <h3 className="text-sm font-bold tracking-wider text-gray-500 uppercase">Active Filters</h3>
                        <button onClick={() => router.get('/marketplace')} className="text-xs text-[var(--primary)] hover:underline">
                            Clear All
                        </button>
                    </div>
                    <div className="bg-background-light grid grid-cols-1 overflow-hidden rounded-lg border border-[var(--buame-border-light)]">
                        {filters.category && filters.category !== 'all' && (
                            <div className="flex items-center justify-between border-b border-[var(--buame-border-light)] p-3">
                                <span className="text-sm text-gray-500">Category</span>
                                <span className="text-sm font-medium capitalize">{filters.category}</span>
                            </div>
                        )}
                        {filters.location && (
                            <div className="flex items-center justify-between border-b border-[var(--buame-border-light)] p-3">
                        ...
                        {filters.condition && filters.condition !== 'all' && (
                            <div className="flex items-center justify-between border-b border-[var(--buame-border-light)] p-3">
                                ...
                                <span className="text-sm font-medium capitalize">{filters.condition.replace('_', ' ')}</span>
                            </div>
                        )}
                        {filters.delivery_available && (
            ...
            {/* Condition Filter */}
            <div>
                <h3 className="mb-4 text-base font-bold">Condition</h3>
            ...
            {/* Delivery Available */}
            <div>
                <h3 className="mb-4 text-base font-bold">Delivery</h3>
                <Label className="group flex cursor-pointer items-center gap-3">
                    <Checkbox
                        ...
                        className="rounded border-gray-300 bg-transparent text-[var(--primary)] focus:ring-[var(--primary)]"
                    />
                    <span className="text-sm font-medium transition-colors group-hover:text-[var(--primary)]">Delivery Available</span>
                </Label>
            </div>
        </aside>
    );
