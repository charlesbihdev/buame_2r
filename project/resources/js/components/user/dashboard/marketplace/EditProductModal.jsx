...
export function EditProductModal({ isOpen, onClose, product }) {
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route('user.dashboard.marketplace.update', product.id), formData, {
            preserveScroll: true,
            onSuccess: () => {
                handleClose();
            },
    ...
    const activeExistingImages = existingImages.filter((img) => !removeImageIds.includes(img.id));
                    ...
                    <div>
                        ...
                        <div className="mt-2 grid grid-cols-4 gap-2 sm:grid-cols-5">
                            ...
                            {existingImages.map((image, index) => {
                                const isMarkedForRemoval = removeImageIds.includes(image.id);
                                return (
                                    <div key={`existing-${image.id}`} className={`group relative aspect-square ${isMarkedForRemoval ? 'opacity-40' : ''}`}>
                                        <img
                            ...
                            {newImagePreviews.map((preview, index) => (
                                <div key={`new-${index}`} className="group relative aspect-square">
                                    ...
                                    <span className="absolute bottom-1 left-1 rounded bg-blue-500 px-1.5 py-0.5 text-xs font-medium text-white">New</span>
                                </div>
                            ))}
