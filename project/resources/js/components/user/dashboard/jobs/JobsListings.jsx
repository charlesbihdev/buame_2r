...
export function JobsListings({ listings }) {
    const handleCreate = () => {
        router.visit(route('user.dashboard.jobs.create'));
    };

    const handleEdit = (id) => {
        router.visit(route('user.dashboard.jobs.edit', id));
    };

    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-[#0d1b0d] dark:text-white">My Job Postings</h3>
                    ...
                    {listings.map((listing) => (
                        ...
                        >
                            <div className="p-4">
                                ...
                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{listing.company_name}</p>
